...
function AdminMiddleSection() {
  ...
  useEffect(() => {
    axios
      .get(`${urlPattern}/api/contacts?page=${currentPage}`)
      .then((response) => {
  ...
  const searchInquiry = async (userEmail) => {
    axios
      .get(`${urlPattern}/api/contacts/search-by-email/${userEmail}/`)
      .then((response) => {
        const userInqueries = response.data.data[0];
        setSearchInqueries(
          Array.isArray(userInqueries) ? userInqueries : [userInqueries],
        );
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="confirmDoneLabel">
                Confirm Action
              </h5>
              <button
                type="button"
